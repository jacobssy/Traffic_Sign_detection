...
from keras.layers import Reshape, Activation, Conv2D, Input, MaxPooling2D, BatchNormalization, Flatten, Dense, Lambda,Average,Add
...
from backend import TinyYoloFeature, FullYoloFeature, MobileNetFeature, SqueezeNetFeature, Inception3Feature, VGG16Feature, ResNet50Feature,YoloV3Feature,S_MobileNet
...
class YOLO(object):
    def __init__(self, architecture,
        ...
        if architecture == 'Inception3':
        ...
        elif architecture == 'YoloV2':
            self.feature_extractor = FullYoloFeature(self.input_size)
        elif architecture == 'YoloTiny':
            ...
            self.feature_extractor = YoloV3Feature(self.input_size)
        elif architecture == 'S_MobileNet':
            self.feature_extractor =S_MobileNet(self.input_size)
        else:
            raise Exception('Architecture not supported! Only support Full Yolo, Tiny Yolo,YoloV3, MobileNet, SqueezeNet, VGG16, ResNet50, and Inception3 at the moment!')

        #print self.feature_extractor.get_output_shape()
        self.grid_h, self.grid_w = self.feature_extractor.get_output_shape()
        #print self.grid_h, self.grid_w
        features = self.feature_extractor.extract(input_image)
        # make the object detection layer
        if architecture == 'YoloV3':
            output1 = Conv2D(self.nb_box * (4 + 1 + self.nb_class),
                            (1,1), strides=(1,1),
                            padding='same',
                            name='conv_output1',
                            kernel_initializer='lecun_normal')(features[0])
            output1 = Reshape((self.grid_h, self.grid_w, self.nb_box, 4 + 1 + self.nb_class))(output1)
            
	    output2 = Conv2D(self.nb_box * (4 + 1 + self.nb_class),
                            (1,1), strides=(2,2),
                            padding='same',
                            name='conv_output2',
                            kernel_initializer='lecun_normal')(features[1])
		
            output2 = Reshape((self.grid_h, self.grid_w, self.nb_box, 4 + 1 + self.nb_class))(output2)
           
	    output3 = Conv2D(self.nb_box * (4 + 1 + self.nb_class),
                            (1,1), strides=(4,4),
                            padding='same',
                            name='conv_output3',
                            kernel_initializer='lecun_normal')(features[2])
            output3 = Reshape((self.grid_h, self.grid_w, self.nb_box, 4 + 1 + self.nb_class))(output3)
            ...
            # layer2 = self.model.layers[3]
            # weights = layer.get_weights()
            # layer3 = self.model.layers[4]
            # weights = layer.get_weights()
            # print len(weights)
        else:
    ...
    def decode_netout(self, netout, obj_threshold=0.25, nms_threshold=0.3):
    ...
    def train(self, train_imgs,     # the list of images to train the model
                    ...
                    coord_scale,
                    class_scale,
                    saved_weights_name='best_weights_yolov2.h5',
                    debug=False):     
        ...
        checkpoint = ModelCheckpoint(saved_weights_name, 
                                     ...
                                     mode='min', 
                                     period=1)
        tb_counter  = len([log for log in os.listdir(os.path.expanduser('./logs_VOC2012/')) if self.architecture in log]) + 1
        tensorboard = TensorBoard(log_dir=os.path.expanduser('./logs_VOC2012/') + self.architecture + str(tb_counter),
                                  histogram_freq=0,
