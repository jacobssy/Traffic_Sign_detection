...
def parse_GTSDB_dataset():
    all_imgs = []
    seen_labels = {}
    filename = '/data4/sunsiyuan/data/FullIJCNN2013/gt.txt'
    data = open(filename).readlines()
    for i in data:
        all_imgs.append(get_train_data_GTSDB(i))
    seen_labels = {'traffic_sign':900}

    return all_imgs,seen_labels

def get_train_data_GTSDB(data):
    content = data.split(';')
    width = '1360'
    height = '800'
    filename = content[0]
...
def parse_annotation(ann_dir, img_dir, labels=[],division=1):
    all_imgs = []
    seen_labels = {}
    count = 0
